...
class Api {
  ...
  Future<List<Bayi>> getiriliskiliBayiListesi(String userdealer, String isletme,String uname) async {
  var response = await http.post(
    Uri.parse('$isletme/Yonetim/GetiliskilisubeListesiApp?dealer=$userdealer&username=$uname'),
  ...
  Future<List<Adisyon>> getirAdisyonListesi(String raporrealtimebayisec, String bastar, String bittar, String kaynak, String durum, String isletme) async {
  var response = await http.post(
    Uri.parse('$isletme/Yonetim/GetirAnalizRaporAdisyonListesi?raporrealtimebayisec=$raporrealtimebayisec&bastar=$bastar&bittar=$bittar&kaynak=$kaynak&durum=$durum'),
    headers: <String, String>{
      'authorization': Constant().getbase64Authentication()
    },
  );
  ...
  if (response.statusCode == 200) {
    ...
    // Veri alındıysa ve dizi boş değilse
    if (dinamik.isNotEmpty) {
      List<Adisyon> adisyon = dinamik.map((json) => Adisyon.fromJson(json)).toList();
      return adisyon;
    } else {
      // Dizi boşsa, boş bir liste döndür
      return [];
  ...
  Future<List<GetirTotalCiro>> getirTotalCiro(String userdealer, String username,String raporrealtimebayisec,String siparisrapordetaytarih,String isletme) async {
    var response = await http.post(Uri.parse('$isletme/Yonetim/GetirTotalCiroApp?userdealer=sbm000&username=$username&raporrealtimebayisec=$raporrealtimebayisec&siparisrapordetaytarih=$siparisrapordetaytarih'),
        headers: <String, String>{
          'authorization': Constant().getbase64Authentication()
        });
